...
classdef LinearValve < FlowCoeffFlowRestriction
    ...
    methods
        ...
        function Cv = getFlowCoefficient(obj)
            Cv = obj.fullyOpenCv .* obj.openAmt;
        end
...
%             end
            ...
            if(isnan(P))
               ME = MException('LinearValve:PNan','P is NaN for given mass flow (Can take any value?)');
               throw(ME);
            end
            
            if(P < 88e3)
               %disp("P TOO LOW!!"); 
               ME = MException('LinearValve:DownstreamPTooLow','Downstream P too low for given mass flow');
               throw(ME);
            end
